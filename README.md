...
> Lưu ý: các % không cộng dồn tuyến tính — request đã cache thì không cần routing.
...
Request nhận vào
        │
        ▼
 [1. Semantic Cache] ──hit──▶ Trả về ngay <200ms
        │miss
        ▼
 [2. LLM chính — Claude Haiku + RAG] ──▶ Stream phản hồi
            │timeout/error
        ▼
 [3. LLM fallback — GPT-4o-mini] ──▶ Stream phản hồi
            │timeout/error
        ▼
 [4. Rule‑based KB — top‑50 intent] ──▶ Template trả lời + cảnh báo
            │không khớp
        ▼
 [5. Static FAQ + Hàng đợi người] ──▶ "Xem tại [link]. Cán bộ sẽ phản hồi trong 30'"
```
...
### 1. Tự đánh giá năng lực (Thang điểm 1‑5)

| Thành viên | Business/Product | Infra/Data/Ops | AI Engineering/Application |
|------------|------------------|----------------|-----------------------------|
| Nông Trung Kiên | 5 | 4 | 3 |
| Trần Ngọc Huy | 4 | 4 | 4 |
| Bùi Thế Công | 4 | 4 | 5 |
| Nghĩa | 4 | 4 | 5 |
| Tiến | 2 | 5 | 4 |
| Sơn | 3 | 5 | 5 |
| **Trung bình nhóm** | **3.8** | **4.3** | **4.3** |

### 2. Điểm mạnh của nhóm

- **Mạnh nhất:** AI Engineering và Infra/Data/Ops (đều 4.3/5)
- **Yếu nhất:** Business/Product (3.8/5)
- **Nhận xét:** Nhóm có thể xây dựng prototype nhanh và chất lượng tốt, nhưng cần bổ sung kỹ năng về khía cạnh kinh doanh/vận hành sản phẩm.
...
|---|---------------------|------|----------|
| 1 | Framework đánh giá RAG (Ragas, TRULENS) — đo faithfulness, answer relevancy | AI Eng/App | Khóa DeepLearning.AI “Building and Evaluating RAG” |
| 2 | LLM Observability (
